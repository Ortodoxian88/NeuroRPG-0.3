...
apiRouter.get('/rooms/:roomId/players', authMiddleware, async (req, res) => {
  try {
    const players = await playersRepository.findByRoom(req.params.roomId);
    res.json(players);
  } catch (error) {
...
apiRouter.post('/rooms/:roomId/players/action', authMiddleware, async (req, res) => {
  try {
    ...
    const player = await playersRepository.findByRoomAndUser(roomId, req.user!.id);
...
apiRouter.get('/rooms/:roomId/messages', authMiddleware, async (req, res) => {
  try {
    ...
    const messages = await messagesRepository.findByRoom(roomId, Number(limit), Number(offset));
    res.json(messages);
  } catch (error) {
